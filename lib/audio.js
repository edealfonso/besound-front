...
export const soundcastingPrepare = async () => {
    console.log({
        function: soundcastingPrepare,
        'player.status': player.status,
        'recorder.status': recorder ? recorder.status : '-',
        recorder: recorder
    });
    ...
    // prepared for double useEffect executions
    if (recorder && !recorder.status == 'stopped') {
        await recorder.stop();
    } else if (!recorder) {
